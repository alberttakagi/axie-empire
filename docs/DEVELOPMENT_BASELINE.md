...
| Treasure | UI ch. 11: regional sets activate after all pieces; 10 listed sets, two locations unconfirmed | Game has 36 sets (12 per saga), partial bonuses, own effects — unchanged. **Resolved 2026-09-21**: set `name`s renamed from originally-made-up fantasy names to real Axie Origins Charm names/art (user decision, chosen from https://app.axieinfinity.com/marketplace/charms/, avoiding omamori/ema-shaped charms); each set also gained an `icon` (bronze/silver/gold PNGs generated from the charm art by tools/treasure-gen, rendered in TreasureScene next to each stage's tier). `id`s/`stageIds`/`bonus` untouched, so earned tiers carry over with no migration needed. |
